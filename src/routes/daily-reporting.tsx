  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight">Daily Reporting</h1>
          <p className="text-sm text-muted-foreground">
            Log daily activity per recruiter. Same recruiter + date overwrites the existing record.
          </p>
        </div>
        <Button className="w-full sm:w-auto shrink-0" onClick={() => { resetForm(); setDialogOpen(true); }}>
                      ...
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            <ReportActions
                              label={`This will permanently delete ${r.recruiter_name}'s report for ${r.date}.`}
                              onEdit={() => startEdit(r)}
          ...
          <div className="md:hidden">
            {isLoading ? (
              ...
              <div className="divide-y divide-[#f3f4f6]">
                {reports.map((r) => {
                  const posCount = r.positions_worked?.length ?? 0;
                  const remark = r.remarks ?? r.notes ?? "";
                  return (
                    <div key={r.id} className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="font-semibold text-foreground">{r.recruiter_name}</div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-3">
                        {[
                          { label: "CVs", value: r.cv_submitted, color: "text-blue-600" },
                          { label: "Interviews", value: r.interviews_scheduled, color: "text-orange-600" },
                          { label: "Joinings", value: r.joinings, color: "text-green-600" },
                        ].map((s) => (
                          <div key={s.label} className="rounded-lg bg-muted/40 py-2 text-center">
                            <div className={`text-lg font-semibold ${Number(s.value) > 0 ? s.color : "text-muted-foreground"}`}>{s.value}</div>
                      ...
                      <div className="flex items-center justify-between gap-3 mt-3">
                        {remark ? (
                          <p className="text-xs text-muted-foreground line-clamp-2 min-w-0">{remark}</p>
                        ) : (
                        ...
                        {posCount > 0 && (
                          <Button
                            ...
                            onClick={() => setViewPositions({ report: r })}
                          >
                            <Eye className="h-3.5 w-3.5" /> {posCount} position{posCount !== 1 ? "s" : ""}
                          </Button>
                        )}
                      </div>
                    </div>
...
}: { label: string; onEdit: () => void; onDelete: () => void }) {
  return (
    <>
      <Button variant="ghost" size="sm" onClick={onEdit} aria-label="Edit report">
        <Pencil className="h-4 w-4" />
      </Button>
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" aria-label="Delete report">
